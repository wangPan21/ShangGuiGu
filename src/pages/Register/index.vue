<template>
  <div class="register-container">
    ...
    <div class="register">
      ...
      <div class="content">
        <label>验证码:</label>
        <input type="text" placeholder="请输入验证码" v-model="captcha" name="captcha"
          v-validate="{ required: true, regex: /^\d{6}$/ }" :class="{ invalid: errors.has('captcha') }">
      ...
      <div class="controls">
        <input name="agree" type="checkbox" :checked="agree" v-validate="{ required: true, 'agree': true }"
          :class="{ invalid: errors.has('agree') }">
        <span>同意协议并注册《蓝海潮用户协议》</span>
  ...
  methods: {
    ...
    async getCode() {
      try {
        ...
        //让store的data等于组件的captcha
        this.captcha = this.$store.state.User.data;
      } catch (error) {
        alert('1223123')
      }
    },
