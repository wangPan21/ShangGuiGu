    ...
    <section class="con">
      ...
      <div class="mainCon">
        ...
        <div class="previewWrap">
          <!--放大镜效果-->
          <Zoom :skuImgeList='skuImageList' />
          <!-- 小图列表 -->
          <ImageList :skuImgeList='skuImageList' />
        </div>
        <!-- 右侧选择区域布局 -->
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="goods in spuSaleAttrList" :key="goods.id">
                <dt class="title">{{ goods.saleAttrName }}</dt>
                <dd changepirce="0" :class="{ active: saleAttrValue.isChecked == 1 }"
                  v-for="saleAttrValue in goods.spuSaleAttrValueList" :key="saleAttrValue.id"
                  @click="changaCtive(saleAttrValue, goods.spuSaleAttrValueList)">
                  {{ saleAttrValue.saleAttrValueName }}</dd>
              </dl>
            </div>
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" class="itxt" v-model="skuNum" @change="changGoodsnum">
...
export default {
  ...
  methods: {
    changaCtive(saleAttrValue, list) {
      //产品的售卖属性值切换高亮
      list.forEach((item) => {
        item.isChecked = 0
      });
